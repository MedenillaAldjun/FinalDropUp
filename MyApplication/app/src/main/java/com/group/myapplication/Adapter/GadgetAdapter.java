...
public class GadgetAdapter extends RecyclerView.Adapter<GadgetAdapter.PostViewHolder> {
    ...
    public GadgetAdapter (Context context, List<Post> List){
        this.List = List;
    ...
    public void onBindViewHolder(@NonNull PostViewHolder holder, int position) {
        ...
        holder.likepic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                firestore.collection("Gadgets/" + postId + "/Likes").document(currentUserId).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if(!task.getResult().exists()){
                            Map<String, Object> likesMap = new HashMap<>();
                            likesMap.put("timestamp", FieldValue.serverTimestamp());
                            firestore.collection("Gadgets/" + postId + "/Likes").document(currentUserId).set(likesMap);
                        }else {
                            firestore.collection("Gadgets/" + postId + "/Likes").document(currentUserId).delete();
                        }
                    }
                });
            }
        ...
        //like color change
        firestore.collection("Gadgets/" + postId + "/Likes").document(currentUserId).addSnapshotListener(new EventListener<DocumentSnapshot>() {
            @Override
        ...
        //likescount
        firestore.collection("Gadgets/" + postId + "/Likes").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
                    ...
                    alert.setTitle("Mark as Sold?")
                            ...
                            .setPositiveButton("I understand, proceed.", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    showPD();
                                    firestore.collection("Gadgets/" + postId + "/Comments").get()
                                            .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                                @Override
                                                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                                    for (QueryDocumentSnapshot snapshot: task.getResult()){
                                                        firestore.collection("Gadgets/" + postId + "/Comments").document(snapshot.getId()).delete();
                                                    }
                                                }
                                            });
                                    firestore.collection("Gadgets/" + postId + "/Likes").get()
                                            .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                                @Override
                                                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                                    for (QueryDocumentSnapshot snapshot: task.getResult()){
                                                        firestore.collection("Gadgets/" + postId + "/Likes").document(snapshot.getId()).delete();
                                                    }
                                                }
                                            });
                                    firestore.collection("Gadgets").document(postId).delete();
