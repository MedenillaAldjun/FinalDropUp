...
public class OrderAdapter extends RecyclerView.Adapter<OrderAdapter.RequestViewHolder> {
    ...
    public OrderAdapter(Activity context,  List<Order> mList){
        this.mList = mList;
    ...
    @NonNull
    @Override
    public OrderAdapter.RequestViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.each_order, parent, false);
        firestore = FirebaseFirestore.getInstance();
        return new OrderAdapter.RequestViewHolder(view);


    }

    @Override
    public void onBindViewHolder(@NonNull OrderAdapter.RequestViewHolder holder, int position1) {
        Order notif = mList.get(position1);
        holder.setItemsPic(notif.getImage2());
        holder.setPostCaption("Buyer Comment: " + notif.getCaption2());

        holder.setRating("Product Rating: " + notif.getRating());
...
// Get the "comments" subcollection of the post
        CollectionReference commentsRef = postRef.collection("BuyerFeedback");
                ...
                alert.setTitle("Delete Notification?")
                        ...
                        .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialogInterface, int i) {
                                showPD();
                                CollectionReference commentsRef = firestore.collection("Users").document(userId).collection("BuyerFeedback");
    ...
    public class RequestViewHolder extends RecyclerView.ViewHolder{
        ...
        public RequestViewHolder(@NonNull View itemView) {
            super(itemView);
            view = itemView;
            delete = view.findViewById(R.id.order_del);

            rate = view.findViewById(R.id.rate2);

            rate.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent rate = new Intent(context, Ratings1.class);
