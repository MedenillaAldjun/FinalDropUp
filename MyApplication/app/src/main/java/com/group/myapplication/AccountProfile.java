...
public class AccountProfile extends AppCompatActivity {
    ...
    ActivityAccountProfileBinding binding;
...
//toolbar navigation
        account = findViewById(R.id.account_toolbar);
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()){
                    if (task.getResult().exists()){
                        ...
                        Glide.with(AccountProfile.this).load(imageUri).into(circleImageView);
                    }
                }
            }
        });
                ...
                firestore.collection("Users").document(Uid).set(map).addOnCompleteListener(new OnCompleteListener<Void>() {
                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if(task.isSuccessful()){
                            ...
                            Toast.makeText(AccountProfile.this, "Profile Settings Saved!", Toast.LENGTH_SHORT).show();
                            startActivity(new Intent(AccountProfile.this, MainActivity.class));
                            finish();
                        }else{

                            Toast.makeText(AccountProfile.this, task.getException().toString(), Toast.LENGTH_SHORT).show();
                        }
                    }
                });
